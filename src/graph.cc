 ...
 */
void LightGraph::createEdgeList(EdgeList &outList,bool ref)
{
...
void LightGraph::sortGraph()
{
    for (NODE_T i=0;i<n;i++)
    {
        std::sort(JA.begin()+IA[i],JA.begin()+IA[i+1]);
    }
}

//it populates the reference and the inverse reference array. 
void LightGraph::createRefArray(bool invRef)
{
    RA.resize(2*m);
    if(invRef) invRA.resize(2*m);
...
void LightGraph::printGraph(NODE_T offset)
{
    for(NODE_T i=0;i<n;i++)
    {
        for(NODE_T j=IA[i];j<IA[i+1];j++)
        {
            std::cout<<i+offset<<" "<<JA[j]<<std::endl; 
        }
    }
}
